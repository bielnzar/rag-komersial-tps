...
from etl.utils import sanitasi_skema_awal, hapus_kolom_hantu, paksa_angka
...
def test_sanitasi_skema_awal():
    df = pd.DataFrame({
        "Unnamed: 0": [1, 2, 3],
        "ValidCol": [4, 5, 6],
        "EmptyCol": [None, None, None]
    })
    df_clean = sanitasi_skema_awal(df)
    
    assert "Unnamed: 0" not in df_clean.columns
    assert "EmptyCol" not in df_clean.columns
    assert "ValidCol" in df_clean.columns

    # Pastikan alias hapus_kolom_hantu berfungsi identik
    df_alias = hapus_kolom_hantu(df)
    assert df_alias.equals(df_clean)
